...
int ActualizaLongitudSerpiente(tipo_serpiente *p_serpiente) {
	...
	contador++; //Incrementamos el contador de la puntuación
...
int CompruebaBotonPulsado (fsm_t* this) {
	int result = 0;
	piLock(SYSTEM_FLAGS_KEY);
	result = (flags & FLAG_TIMER_JUEGO);	// Iniciamos el juego con el botón 5 del teclado matricial o con la S del teclado del ordenador.
	piUnlock(SYSTEM_FLAGS_KEY);
	return result;
}
...
int CompruebaPausa (fsm_t* this){	//Comprobamos que el flag de pausa está activado
	int result = 0;
	piLock(SYSTEM_FLAGS_KEY);
...
void ReseteaJuego (fsm_t* this) {
	...
	ReseteaPantallaSnakePi(p_snakePi->p_pantalla);
	contador = 0;				//Resetamos el contador de puntuación
